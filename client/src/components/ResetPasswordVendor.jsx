...
import imgfolder from "./img/core-img/reset.png";
...
const ResetPasswordVendor = () => {
    ...
    /////////////Email Validation //////////////
      ...
      useEffect(() => {
        const fetchExistingEmails = async () => {
          try {
            const response = await axios.get('http://localhost:4000/api/v1/vendor/');
            const emails = response.data.map(vendor => vendor.email);
    ...
    /////////////Email Validation //////////////
      ...
      const validateForm = () => {
        ...
        // Validate password
        if (!/(?=.*\d)(?=.*[A-Z])(?=.*[!@#$%^&*()_+])(?=.{8,})/.test(vendorData.newPassword)) {
          errors.password =
    ...
    try {
      const response = await fetch('http://localhost:4000/api/v1/vendor/reset_password_vendor', {
        method: 'POST',
        headers: {
    ...
    <div>
        ...
        <div className="background-shape"></div>
             <div className="container">
                ...
                <img className="big-logo" src={imgfolder} alt="" ></img> <p style={{color:'white'}}>Reset Password</p>               
                  ...
                  <input className="form-control" name="email" id="email" value={vendorData.email} onChange={handleChange}  type="email" placeholder="Enter email id"/>
                        </div>
                  ...
                  <input className="form-control" name="question1" id="question1"   value={vendorData.question1} onChange={handleChange} type="text" placeholder="enter the answer"/>
                        </div>
                  ...
                  <input className="form-control" name="question2"id="question2"   value={vendorData.question2} onChange={handleChange} type="text" placeholder="enter the answer"/>
                         </div>
                ...
                <div className="form-group text-start mb-4"><span>Enter New Password</span>
                  ...
                  <input className="input-psswd form-control"   name="newPassword" id="newPassword" value={vendorData.newPassword} onChange={handleChange}  type="password" placeholder="New Password"/>
                </div>
