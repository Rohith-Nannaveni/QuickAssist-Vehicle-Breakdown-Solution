...
const PostBusiness = () => {
  ...
  const postBusinessData = async () => {
    ...
    const vendorEmail = decodeURIComponent(document.cookie.replace(/(?:(?:^|.*;\s*)vendoremail\s*=\s*([^;]*).*$)|^.*$/, '$1'));
  
    try {
      const formPayload = new FormData();
  
      // Append all fields
      formPayload.append('vendoremail', vendorEmail);
      formPayload.append('name', formData.name);
      formPayload.append('type', formData.type);
      formPayload.append('mechanicname', formData.mechanicname);
      formPayload.append('service', formData.service);
      formPayload.append('available', formData.available);
      formPayload.append('locality', formData.locality);
      ...
      const response = await fetch('http://localhost:4000/api/v1/business/', {
        method: 'POST',
        headers: {
          // Do NOT set 'Content-Type' manually when sending FormData; the browser sets it including boundary
          'x-auth-token': token,
        },
        body: formPayload,
      });
      ...
      if (response.ok) {
        ...
        console.error('Error posting business data:', response.status, errorText);
      }
    } catch (error) {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'image') {
      value = e.target.files[0];  // get the actual File object for images
    } else {
        ...
        <div className="profile-wrapper-area py-3">
          <div className="card user-data-card">
            <div className="card-body">
              ...
              <div className="mb-3">
                  <div className="title mb-2"><span>Business Name</span></div>
                ...
                <div className="mb-3">
                <div className="title mb-2"><span>Type</span></div>
                  <select
                ...
                <div className="mb-3">
                  ...
                  <input className="form-control" name="mechanicname" id="mechanicname"
                    value={formData.mechanicname}
                    onChange={handleInputChange}   type="text"/>
                </div>
	  			 <div className="mb-3">
                    <div className="title mb-2"><span>Services</span></div>
                  <input className="form-control" name="service" id="service"
                    value={formData.service}
                    onChange={handleInputChange}   type="text"/>
                </div>
	    			 <div className="mb-3">
                  <div className="title mb-2"><span>Available</span></div>
                  <input className="form-control" name="available" id="available"
                ...
                <div className="mb-3">
                      <div className="title mb-2"><span>Item Image</span></div>
